...
<section class="content">
	<div class="row">
        ...
        <div class="box-body">
            ...
            <?endif;?>
			<form  data-toggle="validator" id="event" action="<?=base_url("event/action/change");?>" method="POST" class="form-horizontal" enctype="multipart/form-data" >
                <input type="hidden" name="user_id" value="<?=$event->user_id?>">
                <input type="hidden" name="event_id" value="<?=$event->id?>">
                <div class="form-group">
                    <label class="col-md-1 control-label"><?=lang('name')?></label>
                    <div class="col-md-7">
                        <input class="form-control" name="title" value="<?=$event->title?>" data-error="<?=lang('empty_msg')?>" required/>
                        ...
                        <select class="form-control" name="type_id" id="type">
                            <?if($event_types):?>
                                <?foreach($event_types as $value):?>
                                    <option value="<?=$value->id?>" <?=($value->id==$event->type_id)?"selected":""?>><?=$value->title?></option>
                                <?endforeach;?>
                            <?endif;?>
                        </select>
                    </div>
                ...
                <div class="form-group" style="margin-top:25px;">
                        ...
                        <select class="form-control  select2" name="location_id" required >
                            <?if($cities):?>
                                ...
                                <?foreach($cities as $value):?>
                                    <option value="<?=$value->id?>" <?=($value->id==$event->location_id)?"selected":""?>><?=$value->city?>, <small style="font-size:7px"><?=$value->sub?></small></option>
                                <?endforeach;?>
                            <?endif;?>
                        </select>
                    </div>
                    ...
                    <div class="col-md-5">
                        <input name="adress" value="<?=$event->adress?>" class="form-control" data-error="<?=lang('empty_msg')?>" required>
                         <div class="help-block with-errors"></div>
                    </div>
	                    ...
	                    <div class="block">
	                        ...
	                        <div class="col-md-5">
	                            <div class="input-group">                               
	                                <input id="start" type='text' name="date_start[]"  value="<?=default_dt($value->date_start)?>" class="form-control date"  data-error="<?=lang('empty_msg')?>"  placeholder="From" readonly  />
	                        ...
	                        <div class="col-md-5" style="margin-bottom: 10px;">
	                             <div class="input-group">
	                                <input id="end" type='text' name="date_end[]"  value="<?=default_dt($value->date_end)?>" class="form-control date" data-error="<?=lang('empty_msg')?>"  placeholder="To" readonly="" />
	                                <span class="input-group-addon"><i class="ion-calendar"></i></span>
	                            </div>
	                             <div class="help-block with-errors"></div>
	                        </div>
                    ...
                    <div class="col-md-3">
                        <div class="btn btn-default btn-file">
                            <i class="fa fa-paperclip"></i> <?=lang('atach')?>
                             <input type="file" name="userfile" size="20" src="<?=base_url($event->img)?>"   />
                            </div>
                            <p><?=lang('img_msg')?></p>
                        <p class="help-block">Max. 800kb</p>
                    </div>
                    <div class="col-md-3">
                    	<img src="<?=base_url($event->img)?>" class="img-responsive">
                    </div>
                </div>
                <div class="form-group">
						<label class=" control-label col-md-1"><?=lang('desc')?></label>
						<div class=" col-md-offset-1 col-md-11">
							<textarea class="form-control" rows="3"  name="description" data-error="<?=lang('empty_msg')?>"><?=$event->description?></textarea>
                             <div class="help-block with-errors"></div>

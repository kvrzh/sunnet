<section class="content-header">
    <h1>Отчету по складу и инвентарю<small></small></h1>
</section>
...
<div class="row">
	...
	<div class="col-md-2">
		<select class="form-control select2" id="gds_id">
			<?if($gds):?>
	...
	<div class="col-md-2">
		<select class="form-control select2" id="move_type">
			<?if($move_type):?>
				<option value="">Все передвижения</option>
					<?foreach($move_type as $key=>$value):?>			
					<option value="<?=$value->id?>"><?=$value->title?></option>
				<?endforeach;?>
			<?endif;?>
		</select>
...
function get_table(){
	var data={
		...
		user_out:$('#user_out option:selected').val(),
	 	gds_id:$('#gds_id option:selected').val(),
	 	move_type:$('#move_type option:selected').val(),
 		start:$('#start').val(),
		end:$('#end').val(),
	};
	$.ajax({
		type:"POST",
		url:"<?=base_url('report/storage_table')?>",
		data:data,

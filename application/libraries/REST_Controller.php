...
abstract class REST_Controller extends CI_Controller {
    ...
    public function _remap($object_called, $arguments)
    {
       ...
       /* if(1==1){
                 $this->response(array('status'=>FALSE,'message'=>$this->rest), self::HTTP_FORBIDDEN);
               }*/
        if($this->rest->controller && $this->rest->controller!=$this->router->class){
             $this->response(array('status'=>FALSE,'message'=>'You have not access for this directory'), self::HTTP_FORBIDDEN);
        }
        if($this->rest->date_end && $this->rest->date_end<date('U')){
             $this->response(array('status'=>FALSE,'message'=>'expired key'), self::HTTP_FORBIDDEN);
        }
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
            ...
            isset($row->date_end) && $this->rest->date_end = $row->date_end;
            isset($row->controller) && $this->rest->controller = $row->controller;
